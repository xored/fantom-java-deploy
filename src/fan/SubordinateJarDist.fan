...
** In contrast to 'JarDist', this class creates Jar without the fantom runtime ('sys' pod, fanx).
** This means, you cannot use this jar, unless other jar with the fantom runtime is in classpath.  
**
class SubordinateJarDist : JdkTask
{
...
//////////////////////////////////////////////////////////////////////////
  ...
  **
  ...
  virtual protected Void preJar()
  {
    log.info("Removing [sys]")
    File sysF := script.devHomeDir + `lib/java/sys.jar`
    (tempDir + `reflect/sys/`).delete
    (tempDir + `res/sys/`).delete
    (tempDir + `etc/sys/`).delete
    (tempDir + `fan/sys/`).delete
    (tempDir + `fanx/`).delete
  }
